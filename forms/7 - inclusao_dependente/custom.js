...
function validarData(data, permiteFuturo, permitePassado) {
  if (data != "") {
    ...
    if (dia > 31 || dia < 1 || mes > 12 || mes < 1) {
      ...
      if (!permiteFuturo && dataInserida > dataAtual) dataValida = false;

      if (!permitePassado && dataInserida < dataAtual) dataValida = false;
    }
...
function preencheUltimoIdDependente() {
  var idExterno = $("#idExterno").val();
  const constraintsDependente = [];
  constraintsDependente.push(DatasetFactory.createConstraint("idExterno", idExterno, idExterno, ConstraintType.MUST));
  var dependente = DatasetFactory.getDataset("busca_proximo_id_dependente", null, constraintsDependente, null);

  if (dependente?.values?.length && !dependente?.values[0]?.ERROR) {
    const number = Number(dependente.values[0].NRO_ULTIMO_DEPENDENTE_CADASTRADO);
    if (!isNaN(number)) $("#dependenteUltimoID").val(number + 1);
    else $("#dependenteUltimoID").val("");
  } else {
    $("#dependenteUltimoID").val("");
  }
}
...
var beforeSendValidate = function (numState, nextState) {
  ...
  if (activity == 0 || activity == 3 || activity == 13) {
    if ($("input[name='tipoSolicitacao']:checked").val() == undefined) {
	  ...
	  const tipoSolicitacao = $("input[name='tipoSolicitacao']:checked").val();
	  if (tipoSolicitacao == "incluir") {
		if ($("#dependenteUltimoID").val() == "") {
		  msg += "Não foi possível buscar o último ID de dependente. Favor verificar se o usuário possui ID Externo no RM.<br>";
		  erro++;
	  	}
	  }
    ...
    if (activity == 0 || activity == 3 || activity == 13) {
      ...
      $("#dependentePCD").change(function () {
        if ($(this).is(":checked")) {
          ...
          if (ultimaOpcaoGrauParentesco == "1") $("#dependenteGrauParentesco").val("3");
        } else {
          $("#IsDependentePCD").val("false");
...
function buscaProximoIDDependente() {}

function createDataset(fields, constraints, sortFields) {
	...
	newDataset.addColumn("DTINIVIGENCIA");
	newDataset.addColumn("DTFIMVIGENCIA");
	...
	var codConsulta = 'FLUIG_INTGR04';
	var codSistema = 'P';
	var codcoligada = 0;
	var usuario;
	var senha;
	var PARAMS;
	var tipoBeneficio = getConstraint(constraints, "tipoBeneficio");
	var codBeneficio;
	if (tipoBeneficio == "valeRefeicao") {
		codBeneficio = ['03']
	}
	if (tipoBeneficio == "planoOdontologico") {
		codBeneficio = ['02']
	}
	if (tipoBeneficio == "previdenciaPrivada") {
		codBeneficio = ['04','05']
	}
	if (tipoBeneficio == "seguroVida") {
		codBeneficio = ['07']
	}
	...
	try {
		...
		if (jsonObj.length > 0) {
			for (var i = 0; i < jsonObj.length; i++) {
				...
				var dataFim = getTagsByName(jsonObj[i], "DTFIMVIGENCIA")[0].replace(new RegExp('(<DTFIMVIGENCIA>|<\/DTFIMVIGENCIA>)', 'g'), '');
				var CODTIPOBENEFICIO = getTagsByName(jsonObj[i], "CODTIPOBENEFICIO")[0].replace(new RegExp('(<CODTIPOBENEFICIO>|<\/CODTIPOBENEFICIO>)', 'g'), '');
				var COD = getTagsByName(jsonObj[i], "CODBENEFICIO")[0].replace(new RegExp('(<CODBENEFICIO>|<\/CODBENEFICIO>)', 'g'), '');

				log.info("TESTEEE")
				log.dir(COD)

				if (COD != "0502" && codBeneficio.indexOf(CODTIPOBENEFICIO) !== -1 && dataFim == "" || new Date(dataFim) > new Date()) {
					var row = [];
